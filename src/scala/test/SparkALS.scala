...
object SparkALS {
  ...
  def generateR(): RealMatrix = {
    val mh = randomMatrix(M, F)  //M행렬 생성(item개수, feature개수)
    val uh = randomMatrix(U, F)  //U행렬 생성(user수, feature개수)
    mh.multiply(uh.transpose())  //U.i^T * M.j = r.ij
  }

  def rmse(targetR: RealMatrix, ms: Array[RealVector], us: Array[RealVector]): Double = {
    val r = new Array2DRowRealMatrix(M, U)  //0행렬 생성(M,U)
    for (i <- 0 until M; j <- 0 until U) {
      r.setEntry(i, j, ms(i).dotProduct(us(j)))  //ms * us 스칼라곱 진행한 결과를 r.ij에 setting
    }
    val diffs = r.subtract(targetR)  //targetR을 뺀 값을 반환
    var sumSqs = 0.0
    for (i <- 0 until M; j <- 0 until U) {
      val diff = diffs.getEntry(i, j)
      sumSqs += diff * diff
    }
    math.sqrt(sumSqs / (M.toDouble * U.toDouble)) //제곱합계평균 제곱근 반환
  }

  def update(i: Int, m: RealVector, us: Array[RealVector], R: RealMatrix) : RealVector = {
    val U = us.length
    val F = us(0).getDimension  //feature값 get
  ...
  def main(args: Array[String]) {
    ...
    sc.setLogLevel("ERROR")  //loglevel setting
